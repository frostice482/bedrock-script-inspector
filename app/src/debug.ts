...
namespace BedrockInspector {
    ...
    ws.addEventListener('message', ({data: raw}) => {
        const [name, intData] = JSON.parse(raw) as BedrockInterpreterType.CrossEventData

        if (name === 'bedrock_events') {
            for (const [name, data] of intData)
                bedrockEvents.dispatchEvent(new CustomEvent(name, { detail: data }))
        }

        events.dispatchEvent(new CustomEvent(name, { detail: intData }))
    })
}
